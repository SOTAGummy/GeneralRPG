...
class InjectionTableContainer(playerInv: IInventory, val te: TileEntityInjectionTable) : Container() {
	init {
		...
		this.addSlotToContainer(object: SlotItemHandler(inventory, 0, 50, 20){
		...
		for (i in 0..2) {
			for (j in 0..8) {
				this.addSlotToContainer(Slot(playerInv, i * 9 + j + 9, 8 + j * 18, 84 + i * 18))
			}
		}

		repeat(9) {
			this.addSlotToContainer(Slot(playerInv, it, 8 + it * 18, 142))
		}
	}
	...
	override fun transferStackInSlot(playerIn: EntityPlayer, index: Int): ItemStack {
		...
		if (slot != null && slot.hasStack) {
			...
			var containerSlots = inventorySlots.size - playerIn.inventory.mainInventory.size;
